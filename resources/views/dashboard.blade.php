    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form method="GET" action="{{ route('dashboard') }}" class="row mb-4">
                    ...
                    <div class="col-md-3">
                        <select name="submitter" class="form-control">
                            ...
                            @foreach($submitters as $submitter)
                                <option value="{{ $submitter->id }}" {{ request('submitter')==$submitter->id ? 'selected':'' }}>
                                    {{ $submitter->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                @endif
                <div class="col-md-3">
                    <input type="date" name="submission_date" value="{{ request('submission_date') }}" class="form-control">
                </div>
                    ...
                    @foreach($projects as $project)
                        ...
                        @if(auth()->user()->role==='admin') 
                            ...
                            {{-- Pending projects only --}}
                                @if($project->status ==0)
                                    <input type="checkbox" name="project_ids[]" value="{{ $project->id }}" class="projectCheckbox">
                                @endif
                            </td> 
                        @endif   
                            <td>{{ $project->title }}</td>
                            ...
                            @if(auth()->user()->role === 'admin')
                                <td>
                                    <a href="{{ route('projects.approve', $project->id) }}" class="btn btn-success btn-sm">Approve</a>
